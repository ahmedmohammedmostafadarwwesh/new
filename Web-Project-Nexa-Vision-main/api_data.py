...
def get_sample_rows() -> List[DashboardRow]:
    return [
...
def safe_distance() -> float | None:
    try:
        return float(ultrasonic.get_distance())
    except Exception:
        return None


def safe_encoder_count() -> int | None:
    try:
        return int(encoder.get_count())
    except Exception:
        return None
...
@app.get("/api/full-status", response_model=FullStatus)
def get_full_status():
    return FullStatus(
        command=current_command,
        detections=latest_detections,
        distance=safe_distance(),
        encoder_count=safe_encoder_count(),
